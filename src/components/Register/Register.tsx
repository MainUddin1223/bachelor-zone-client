  ...
  return (
      <div className={Styles.container}>
          <Row gutter={[20,20]} align='middle' justify='center'>
              <Col xs={24} md={12}>
                  <Image src={register_img} width={50} height={50} alt='login_pic' layout='responsive'/>
              </Col>
              <Col xs={24} md={12}>
                          ...
                          <div className={Styles.input_container}>
                              <p>{ getLang == 'বাং' ?'ডকুমেন্টের ধরন':'Document type'}</p>
                                <Radio.Group onChange={(e: RadioChangeEvent) => {
                                    ...
                                    <Radio value={'nid'}>{ getLang == 'বাং' ?'এনআইডি':'NID'}</Radio>
                                    <Radio value={'brn'}>{ getLang == 'বাং' ?'জম্মনিবন্ধন':'Birth Reg'}</Radio>
                                    </Radio.Group>
                          </div>
                          <div className={Styles.input_container}>
                              <p>{ getLang == 'বাং' ?'এনআইডি/জম্মনিবন্ধন নাম্বার':'NID Number / Birth reg number'}</p>
                              <Input placeholder="2196415318" type='number'/>
                          </div>
                          <div>
                              
                          <Button type='primary' style={{display:'block' ,margin:'5px auto'}} >{getLang === 'বাং'?'রেজিস্ট্রেশন করুন':"Register"}</Button>
                          </div>
                      </div>
                      <div className={Styles.input_container}>  
